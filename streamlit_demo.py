...
from app.constants.constants import (FONT, HEIGHT,
                                     STREAMLIT_OUTPUT_DIR, TEXT_ORIGIN,
                                     TIMER_DURATION, WIDTH)
from app.utils.commons import create_directory, generate_random_id, move_images


detect_model_path = os.path.join(
    os.path.dirname(__file__), "app/model/best.pt")
    ...
    if i % 5 == 0:
        ...
        if captured:
            ...
            if time.time() - capture_time <= TIMER_DURATION:
                cv2.putText(img, "Captured!", TEXT_ORIGIN,
                            FONT, 0.8, (128, 255, 0), 1)
        ...
        cv2.putText(
            img, "Document Not Detected!!", TEXT_ORIGIN, FONT, 0.8, (
                0, 0, 255), 1
        )
